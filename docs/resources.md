...
| Batch exports                | ✅ `projects/{id}/batch_exports`               | ✅                  | Identity marker in `name`. Declarative `paused`. Destination credentials via `secret("ENV_VAR")` (masked on read; #89 convention). Inline-credential destinations (AwsS3 / S3Compatible / Snowflake); integration-backed types (Databricks / AzureBlob / BigQuery / Postgres / Redshift) rejected with a validation error. |
