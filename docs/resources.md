...
| Surveys                  | ✅ `projects/{id}/surveys`                  | ✅                  | Identity via `iac:surveys:<key>` marker in `description`. Declarative lifecycle (`draft` / `running` / `stopped`) drives `launch` / `stop`. References feature flags by key (`linkedFlag` / `targetingFlag`). Questions (open / link / rating / single- & multiple-choice), appearance, and conditions supported; the auto-generated internal targeting flag (`targeting_flag_filters`) is not — declare a `linkedFlag` instead. |
