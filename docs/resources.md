...
| Early access features    | ✅ `projects/{id}/early_access_feature`     | ✅                  | Identity via `iac:early-access-features:<key>` marker in `description`. Links a feature flag by key (`featureFlag`); `stage` (concept / alpha / beta / general-availability / …) is a declarative field. Auto-flag creation is not supported — declare and link the flag. |
