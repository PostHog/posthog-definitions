...
// Re-exported for backward compatibility — callers that previously imported
// SafetyViolationError from `apply/execute.js` keep working. The class itself
// lives in `apply/errors.js` so resource pipelines can import it without
// pulling in the apply pipeline, which would form an import cycle:
//   pipeline.ts → execute.ts → resources/index.ts → pipeline.ts
export { SafetyViolationError } from "./errors.js";
