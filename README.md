...
Infrastructure-as-code for PostHog. Define dashboards, insights, feature flags, actions, endpoints, event definitions, property groups, experiments, experiment holdouts, experiment saved metrics, and surveys in TypeScript, then sync them to a PostHog project with one command.
...
1. **Authenticate** — create a personal API key ([US](https://us.posthog.com/settings/user-api-keys) / [EU](https://eu.posthog.com/settings/user-api-keys)) with these scopes:
   ...
   - `survey:read`, `survey:write` (surveys reference feature flags, so `feature_flag:*` above is also needed)
